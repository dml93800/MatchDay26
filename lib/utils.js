...
export function getGroupStandings(matches) {
  ...
  const usHour = parseInt(h)
  const frStr = `${String(frHour).padStart(2,'0')}:${m}`
  const usStr = `${String(usHour).padStart(2,'0')}:${m}`
  const period = usHour >= 12 ? 'PM' : 'AM'
  const us12 = usHour > 12 ? usHour - 12 : usHour === 0 ? 12 : usHour
  return `${frStr} (FR) · ${us12}:${m} ${period} (US)`
}
}
