...
export default function MatchDetail({ match, onClose }) {
  ...
  useEffect(() => {
    fetchStats()
    if (match.score) generateAnalysis()
  }, [])
  ...
  function getStat(arr, type) { return arr.find(s => s.type === type)?.value || 0 }

  const statItems = [
    { label: 'Possession', home: getStat(homeStats, 'Ball Possession'), away: getStat(awayStats, 'Ball Possession') },
    ...
    <div style={{ position:'fixed', inset:0, background:'var(--bg)', zIndex:200, overflowY:'auto', maxWidth:480, margin:'0 auto' }}>
      ...
      {activeTab === 'resume' && (
        <div style={{ padding:16 }}>
          ...
          {loadingAI && (
            <div style={{ display:'flex', alignItems:'center', gap:8, color:'var(--text3)', fontSize:13, padding:'8px 0' }}>
              <span className="ai-dot" /><span className="ai-dot" /><span className="ai-dot" />
              <span>L'IA rédige le résumé...</span>
            </div>
          )}
          {analysis && (
      ...
      {activeTab === 'stats' && (
        <div style={{ padding:16 }}>
          ...
          {!loadingStats && !stats?.statistics?.length && (
            <div style={{ textAlign:'center', color:'var(--text3)', padding:'2rem', fontSize:13 }}>Stats non disponibles pour ce match</div>
          )}
                ...
                return (
                  <div key={label} style={{ marginBottom:14 }}>
                    <div style={{ display:'flex', justifyContent:'space-between', marginBottom:5 }}>
                      <span style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:18, color:'#fff' }}>{home}</span>
                      <span style={{ fontSize:11, color:'var(--text3)', alignSelf:'center' }}>{label}</span>
                      <span style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:18, color:'#fff' }}>{away}</span>
                    </div>
